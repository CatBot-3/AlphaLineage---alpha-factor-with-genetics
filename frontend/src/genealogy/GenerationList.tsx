        ...
        return (
          <div className="gen-block" key={gen.generation} data-testid="gen-block">
            <button
              ...
              onClick={() => setOpenGens((items) => toggle(items, gen.generation))}
            >
              <span className="gen-caret" aria-hidden="true">{genOpen ? "▾" : "▸"}</span>
              <span className="gen-title">Generation {gen.generation}</span>
              <span className="gen-best">best {fmt(gen.bestFitness)}</span>
            </button>

            {genOpen && gen.groups.map((group) => {
              ...
              return (
                <div className="method-group" key={key} data-testid="method-group">
                  <button
                    ...
                    onClick={() => setOpenGroups((items) => toggle(items, key))}
                  >
                    <span className="gen-caret" aria-hidden="true">{groupOpen ? "▾" : "▸"}</span>
                      ...
                      {group.members.map((member) => (
                        <li className="member-row" key={member.id} data-testid="member-row">
                          <button
                            ...
                            aria-label={`Select formula ${member.id}`}
                          ...
                          {onSave && (
                            <button
                              ...
                              onClick={() => {
                                const node = lineage.nodes.find((item) => item.id === member.id);
                                if (node) onSave(node);
                              }}
                            >
                              Save
